...
namespace DWIS.Service.WOBCorrections.ModelSharedOut
{
    /// <summary>
    ...
    class Program
    {
        ...
        private static readonly string JSON_BUNDLE = "WOBCorrectionsMergedModel.json";
        private static readonly string CSHARP_MODEL = "WOBCorrectionsMergedModel.cs";
        private static readonly string PSEUDO_CTOR = "PseudoConstructors.cs";

        // ### BEGIN CODE SPECIFIC TO ModelSharedOut 3/3 ###
        private static readonly string NAMESPACE = "DWIS.Service.WOBCorrections.ModelShared"; // should be the same as for ModelSharedIn to avoid type name collision
        private static readonly string MODELSHARED_FOLDER = "DWIS.Service.WOBCorrections.ModelSharedOut";
        private static readonly string JSON_OUTPUT_FOLDER = "DWIS.Service.WOBCorrections.Service" + Path.DirectorySeparatorChar + "wwwroot" + Path.DirectorySeparatorChar + "json-schema";
        ...
        static async Task Main()
        {
            ...
            if (directory != null)
            {
                ...
                do
                {
                    ...
                    {
                        if (existsFile)
                            Console.WriteLine("Run again for proper creation of constructor class.");
                    }
                    lock (lock_)
                    {
                        finished = finished_;
                ...
                Thread.Sleep(100);
                //if (existsFile)
                //    CreatePseudoConstructors(directory);
            }
            else
            {
        ...
        static async Task<bool> GenerateModelFromDependencies(DirectoryInfo directory)
        {
        ...
        static bool CreatePseudoConstructors(DirectoryInfo directory)
        {
            string icollecionString = "System.Collections.Generic.ICollection`1[";
            string nspace = "DWIS.Service.WOBCorrections.ModelShared";
            var classes = from t in Assembly.GetExecutingAssembly().GetTypes()
                          where t.IsClass && t.Namespace == nspace
                          select t;

            string identationProperty = "\n\t\t\t\t";
            classes = classes.ToList();
            string pseudoConstructorsText = "namespace " + nspace + "\n{\n\tpublic class PseudoConstructors\n\t{";
            foreach (var q in classes)
            {
                    ...
                    !q.Name.Contains("Light"))
                {
                    ...
                    foreach (var p in q.GetProperties())
                    {
                        ...
                        {
                            //Check if it is an ICollection
                            bool listPrefix = p.ToString()!.Contains("ICollection");
                            bool isFromNamespace = (p.ToString()!.Contains(nspace));
                            ...
                            string propertyName = identationProperty + p.Name + " = ";
                            string defaultValueString = "";
                            //Remove namespace from property name
                            if (isFromNamespace)
                            {
                                int index = typeName.IndexOf(nspace + ".");
                                typeName = (index < 0) ? typeName : typeName.Remove(index, nspace.Length + 1);
                            }
                            //Check if there is a collection for flow control
                            if (listPrefix)
                            {
                                //Remove ICollection info
                                int index = typeName.IndexOf(icollecionString);
                                typeName = (index < 0) ? typeName : typeName.Remove(index, icollecionString.Length);
                            ...
                            if (isEnum)
                            {
                                defaultValueString = $"({typeName})0,";
                            }
                            else
                            {
                                //Check for dates from MetaInfo
                                if (p.Name == "CreationDate" || p.Name == "LastModificationDate")
                                {
                                    defaultValueString = "DateTimeOffset.UtcNow,";
                                }
                                else if (p.ToString()!.Contains("Nullable"))//Check for nullables
                                {
                                    defaultValueString = "null,";
                                }
                                else if (isFromNamespace)//Check if it belongs no the namespace
                                {
                                    //Call ConstructMyClass() constructor
                                    defaultValueString = $"Construct{typeName}(),";
                                }
                                else if (p.PropertyType.ToString() == "System.Guid") //Check if it is a Guid
                                {
                                ...
                                {
                                    //Check if it is a value type and fill it with the default value.                     
                                    object defaultValue = Activator.CreateInstance(p.PropertyType);
                            ...
                            //If the "ICollection<>" boolean was true, create a list with a single, empty, entry
                            if (listPrefix)
                            {
                                string listDefaultValue = $"new List<{typeName}>" + " \n\t\t\t\t{\n\t\t\t\t\t" + defaultValueString + " \n\t\t\t\t},";
                                propertiesText += propertyName + listDefaultValue;
                            }
            ...
            pseudoConstructorsText += "\n\t}\n}";
            File.WriteAllText(directory.FullName + Path.DirectorySeparatorChar + MODELSHARED_FOLDER + Path.DirectorySeparatorChar + PSEUDO_CTOR, pseudoConstructorsText);
            return true;
        }
